...
namespace TacticsToolkit
{
    ...
    public class MovementController : MonoBehaviour
    {
        ...
        private void MoveAlongPath()
        {
            ...
            activeCharacter.transform.position = Vector2.MoveTowards(activeCharacter.transform.position, path[0].transform.position, step);
            ...
            if (Vector2.Distance(activeCharacter.transform.position, path[0].transform.position) < 0.0001f)
            {
                //last tile
                if (path.Count == 1)
        ...
        public void FocusedOnNewTile(GameObject focusedOnTile)
        {
            ...
            if (movementModeEnabled && inRangeTiles.Contains(focusedTile) && !isMoving && showAttackRange && displayAttackRange)
